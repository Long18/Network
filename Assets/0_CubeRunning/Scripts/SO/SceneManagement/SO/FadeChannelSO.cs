...
public class FadeChannelSO : DescriptionBaseSO
{
    ...
    private void Fade(bool fadeIn, float duration, Color color)
    {
        if (OnEventRaised == null)
            Debug.LogWarning("A Fade was requested, but nobody picked it up. " +
                             "Check why there is no FadeController already present, " +
                             "and make sure it's listening on this Fade Event channel.");
        else
