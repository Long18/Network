...
public class ReplaceTool : EditorWindow
{
    ...
    [MenuItem("EditorTools/Replace with Prefab")]
    ...
    private void OnGUI()
    {
        ...
        {
            EditorGUILayout.Separator();
            EditorGUILayout.LabelField("Select objects in the hierarchy to replace them",
    ...
    {
        InitDataIfNeeded();
        SelectionMode objectFilter = SelectionMode.Unfiltered ^
                                     ~(SelectionMode.Assets | SelectionMode.DeepAssets | SelectionMode.Deep);
    ...
    private void ReplaceSelectedObjects(GameObject[] objectToReplace, GameObject replaceObject)
    {
        ...
        Debug.LogFormat("[Replace Tool] {0} objects replaced on scene with {1}", objectToReplace.Length,
            replaceObject.name);
    }
}
