...
namespace StateMachine.Editor
{
    ...
    internal class TransitionTableEditor : UnityEditor.Editor
    {
                    ...
                    // State Header Buttons
                    {
                        ...
                        if (Button(buttonRect, "SceneViewTools"))
                        {
                            if (_cachedStateEditor == null)
                                _cachedStateEditor =
                                    CreateEditor(transitions[0].SerializedTransition.FromState.objectReferenceValue,
                                        typeof(StateEditor));
                            else
                                CreateCachedEditor(transitions[0].SerializedTransition.FromState.objectReferenceValue,
                                    typeof(StateEditor), ref _cachedStateEditor);
        ...
        {
            var state = _transitions.GetArrayElementAtIndex(index).FindPropertyRelative("FromState")
        ...
        private bool DoToggleAndSort(Object state, int index)
        {
            ...
            for (int i = 0; i < _transitions.arraySize; i++)
            {
                if (_transitions.GetArrayElementAtIndex(i).FindPropertyRelative("FromState").objectReferenceValue ==
                    state)
                {
                    ret = true;
                    if (i > index)
            ...
            out int toIndex)
        {
            ...
            toIndex = _transitionsByFromStates[fromIndex].FindIndex(
                transitionHelper => transitionHelper.SerializedTransition.ToState.objectReferenceValue ==
        ...
        private void GroupByFromState()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                {
                    Debug.LogError("Transition with invalid state found in table " +
